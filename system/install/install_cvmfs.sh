...
dnf install --installroot=$BUILD_DIR --releasever=/ --nodocs /cernvm-system-default-latest-x86_64.rpm

wget -P / http://ecsft.cern.ch/dist/cernvm/five/rpms/latest/cernvm-config-default-latest-x86_64.rpm
dnf install --installroot=$BUILD_DIR --releasever=/ --nodocs /cernvm-config-default-latest-x86_64.rpm
